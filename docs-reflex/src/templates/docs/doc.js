...
import {Box} from '@reflexjs/components';

const getNextDoc = (uri, allDocs) => {
    console.log(uri, allDocs);
    const uniqueDocs = _.uniqBy(allDocs, function (doc) {
        return doc.url;
    });
...
export const getTag = (tag, componentProps, __themeKey) => {
    ...
    const nodeName = tag.charAt(0).toUpperCase() + tag.slice(1);
    const possibleComponent = reflexComponents[nodeName];
...
const ParseHtml = html => {
    ...
    export const Doc = ({id, title, excerpt, content, uri}) => {
    ...
    `);
        ...
        console.log({id, nextDoc, prevDoc});
